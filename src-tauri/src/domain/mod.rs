use serde::{Deserialize, Serialize};
use thiserror::Error;

const MAX_TITLE_LEN: usize = 200;
const MAX_DESCRIPTION_LEN: usize = 2000;
const MAX_COLUMN_NAME_LEN: usize = 50;

#[derive(Error, Debug)]
pub enum DomainError {
    #[error("Validation error: {0}")]
    Validation(String),
    #[error("Entity not found: {0}")]
    NotFound(String),
    #[error("Duplicate column name: {0}")]
    DuplicateColumnName(String),
}

#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct Task {
    pub id: String,
    pub title: String,
    pub description: Option<String>,
    pub column_id: String,
    pub order: u32,
    pub created_at: chrono::DateTime<chrono::Utc>,
    pub updated_at: chrono::DateTime<chrono::Utc>,
    pub completed_at: Option<chrono::DateTime<chrono::Utc>>,
}

impl Task {
    pub fn new(
        title: String,
        description: Option<String>,
        column_id: String,
        order: u32,
    ) -> Result<Self, DomainError> {
        if title.is_empty() {
            return Err(DomainError::Validation("Title cannot be empty".to_string()));
        }
        if title.len() > MAX_TITLE_LEN {
            return Err(DomainError::Validation(format!(
                "Title cannot exceed {} characters",
                MAX_TITLE_LEN
            )));
        }
        if let Some(ref desc) = description {
            if desc.len() > MAX_DESCRIPTION_LEN {
                return Err(DomainError::Validation(format!(
                    "Description cannot exceed {} characters",
                    MAX_DESCRIPTION_LEN
                )));
            }
        }
        let now = chrono::Utc::now();
        Ok(Self {
            id: uuid::Uuid::new_v4().to_string(),
            title,
            description,
            column_id,
            order,
            created_at: now,
            updated_at: now,
            completed_at: None,
        })
    }
}

#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct Column {
    pub id: String,
    pub name: String,
    pub order: u32,
}

impl Column {
    pub fn new(name: String, order: u32) -> Result<Self, DomainError> {
        if name.is_empty() {
            return Err(DomainError::Validation(
                "Column name cannot be empty".to_string(),
            ));
        }
        if name.len() > MAX_COLUMN_NAME_LEN {
            return Err(DomainError::Validation(format!(
                "Column name cannot exceed {} characters",
                MAX_COLUMN_NAME_LEN
            )));
        }
        Ok(Self {
            id: uuid::Uuid::new_v4().to_string(),
            name,
            order,
        })
    }
}

#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct Board {
    pub id: String,
    pub name: String,
    pub columns: Vec<Column>,
    pub schema_version: u32,
}

impl Board {
    pub fn new(name: String, columns: Vec<Column>) -> Result<Self, DomainError> {
        if name.is_empty() {
            return Err(DomainError::Validation(
                "Board name cannot be empty".to_string(),
            ));
        }
        let board = Self {
            id: uuid::Uuid::new_v4().to_string(),
            name,
            columns,
            schema_version: 1,
        };
        board.validate()?;
        Ok(board)
    }

    fn validate(&self) -> Result<(), DomainError> {
        let mut seen_names: std::collections::HashSet<String> = std::collections::HashSet::new();
        for col in &self.columns {
            if !seen_names.insert(col.name.to_lowercase()) {
                return Err(DomainError::DuplicateColumnName(col.name.clone()));
            }
        }
        Ok(())
    }
}

#[cfg(test)]
mod tests {
    use super::*;

    #[test]
    fn task_new_valid() {
        let task = Task::new(
            "Test task".to_string(),
            Some("Description".to_string()),
            "col-1".to_string(),
            0,
        )
        .unwrap();
        assert!(!task.id.is_empty());
        assert_eq!(task.title, "Test task");
        assert_eq!(task.description, Some("Description".to_string()));
    }

    #[test]
    fn task_new_empty_title() {
        let result = Task::new("".to_string(), None, "col-1".to_string(), 0);
        assert!(result.is_err());
    }

    #[test]
    fn task_new_title_too_long() {
        let long_title = "a".repeat(MAX_TITLE_LEN + 1);
        let result = Task::new(long_title, None, "col-1".to_string(), 0);
        assert!(result.is_err());
    }

    #[test]
    fn task_new_description_too_long() {
        let long_desc = "a".repeat(MAX_DESCRIPTION_LEN + 1);
        let result = Task::new("Title".to_string(), Some(long_desc), "col-1".to_string(), 0);
        assert!(result.is_err());
    }

    #[test]
    fn task_new_with_no_description() {
        let task = Task::new("Test task".to_string(), None, "col-1".to_string(), 0).unwrap();
        assert_eq!(task.description, None);
    }

    #[test]
    fn column_new_valid() {
        let col = Column::new("To Do".to_string(), 0).unwrap();
        assert!(!col.id.is_empty());
        assert_eq!(col.name, "To Do");
    }

    #[test]
    fn column_new_empty_name() {
        let result = Column::new("".to_string(), 0);
        assert!(result.is_err());
    }

    #[test]
    fn column_new_name_too_long() {
        let long_name = "a".repeat(MAX_COLUMN_NAME_LEN + 1);
        let result = Column::new(long_name, 0);
        assert!(result.is_err());
    }

    #[test]
    fn board_new_valid() {
        let col = Column::new("To Do".to_string(), 0).unwrap();
        let board = Board::new("My Board".to_string(), vec![col]).unwrap();
        assert!(!board.id.is_empty());
        assert_eq!(board.name, "My Board");
        assert_eq!(board.columns.len(), 1);
    }

    #[test]
    fn board_new_empty_name() {
        let result = Board::new("".to_string(), vec![]);
        assert!(result.is_err());
    }

    #[test]
    fn board_duplicate_column_names() {
        let col1 = Column::new("To Do".to_string(), 0).unwrap();
        let col2 = Column::new("To Do".to_string(), 1).unwrap();
        let result = Board::new("Board".to_string(), vec![col1, col2]);
        assert!(result.is_err());
        assert!(matches!(
            result.unwrap_err(),
            DomainError::DuplicateColumnName(_)
        ));
    }

    #[test]
    fn board_case_insensitive_column_names() {
        let col1 = Column::new("To Do".to_string(), 0).unwrap();
        let col2 = Column::new("to do".to_string(), 1).unwrap();
        let result = Board::new("Board".to_string(), vec![col1, col2]);
        assert!(result.is_err());
    }

    #[test]
    fn task_order_non_negative() {
        let task = Task::new("Test".to_string(), None, "col-1".to_string(), 0).unwrap();
        assert_eq!(task.order, 0);
        let task2 = Task::new("Test2".to_string(), None, "col-1".to_string(), 100).unwrap();
        assert_eq!(task2.order, 100);
    }
}
